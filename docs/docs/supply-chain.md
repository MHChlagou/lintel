...
2. **Runs the scanner's version command** and compares the output against the pinned version string in `lintel.yaml`.
...
4. Update the `sha256` block in `lintel.yaml`.
...
The `lintel` binary itself is built by a SHA-pinned GitHub Actions workflow (`release.yml`), cross-compiled with `CGO_ENABLED=0`, and signed with [Sigstore keyless signing](https://www.sigstore.dev/). To verify a release binary:
