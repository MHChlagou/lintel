...
**Allowlist**
:   A persistent list of (scanner, rule, path glob, reason) entries that suppress matching findings. Lives in `.aegis/allowlist.yaml`.

**Baseline**
:   A JSON snapshot of every finding at a point in time. Findings present in the baseline are excluded from the gate on future runs; new findings are not. Lives in `.aegis/baseline.json`.

**Check**
:   One of Aegis's five logical check categories: `secrets`, `malicious_code`, `dependencies`, `lint`, `format`. Each check has one or more scanners per stack.
...
**Override**
:   An operator-initiated bypass of the gate for a single run, gated by a mandatory reason and recorded in `.aegis/overrides.log`.

**Pin**
:   The combination of `version` + per-platform `sha256` fields in `aegis.yaml` that identifies the exact scanner binary Aegis is willing to execute.

**`protect_secrets`**
:   Top-level config flag (default `true`) that prevents the `secrets` check from being disabled by any mechanism.

**Scanner**
:   An external tool (`gitleaks`, `biome`, …) that Aegis coordinates. Aegis does not contain its own rules or databases.
