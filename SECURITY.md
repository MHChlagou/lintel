...
Thanks for helping keep Lintel and its users safe.
...
- A minimal reproduction: repo layout, `lintel.yaml`, commands, and observed
  vs. expected output.
...
- The `lintel` binary and all code under this repository.
...
- Bugs in third-party scanner binaries (`gitleaks`, `opengrep`, `osv-scanner`,
  `biome`, `ruff`, `golangci-lint`, `gofmt`, `shellcheck`). Lintel is a
...
If you run Lintel in CI or as a hook:

- Keep `strict_versions: true` in `lintel.yaml` so binary hashes are checked on
  every invocation.
- Pin `lintel` itself to a released version with a published checksum; do not
